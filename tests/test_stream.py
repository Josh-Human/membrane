...
class TestStreamGet:
    ...
    def test_get_composition(self):
        assert self.stream.composition == {"CO2": 0.5, "N2": 0.5}
    ...
    def test_get_component(self):
        assert self.stream.composition["CO2"] == 0.5
        with pytest.raises(KeyError):
            self.stream.composition["H2O"]
...
class TestStreamSet:
    ...
    def test_set_composition_dict(self):
        self.stream.composition = {"CO2": 0.25, "N2": 0.75}

        assert self.stream.composition == {"CO2": 0.25, "N2": 0.75}

    def test_set_composition_dict_incorrect_length(self):
        stream = StreamConstructor(
            set_up("data.json", "composition", {"CO2": 0.25, "N2": 0.25, "H2O": 0.5})
        ).stream
    ...
    def test_set_composition_list_correct_length(self):
        stream = StreamConstructor(set_up("data.json")).stream
    ...
    def test_set_composition_list_incorrect_length(self):
        stream = StreamConstructor(
            set_up("data.json", "composition", {"CO2": 0.25, "N2": 0.25, "H2O": 0.5})
        ).stream
